...
<main>
    ...
    <section id="contact">
        ...
        <form method="post" action="#received">
            ...
            <label>HTTP Mailbox<input type="url" name="mailbox"
                    value="<?= htmlspecialchars(
                                filter_input(INPUT_POST, "mailbox", FILTER_SANITIZE_URL) ?? ''
                            ) ?>" />
            </label>
        ...
        if ($_SERVER["REQUEST_METHOD"] === "POST" && $subject && $message && $email && $name && $urgency) {
            ...
            write("messages", $article = Chitch\message($subject, 'en', $message, $email, $name, $urgency, $bot));
            //notification?
