...
use function Chitch\{batch, read, write, chitchmail};
...
if (read('members') !== [] && php_sapi_name() !== "cli-server") {
...
<p>Operating System is <?= PHP_OS === 'Linux' ? '' : 'NOT' ?> supported for production. Other Operating systems are only supported for Development purposes.
...
<p>You PHP version, <?= $version = phpversion() ?>, is compatible with the PHP above the version 8.4.0
    <?=
    version_compare(phpversion(), '8.4.0', '<') ? ', which it is <strong>not</strong>' : ', your PHP version is supported!'
    ?>.
...
<table>
    ...
    <tbody>
        <tr>
        <td>PHP version 8.4
        <td><?= $version = phpversion()?>
        <td><?= version_compare(phpversion(), '8.4.0', '<') ? 'Version lower' : 'Supported version'?>
        </tr>
        <tr>
            <?= Chitch\tree('td', 'Filter Extension', $filterext= extension_loaded('filter'),  $filterext ? 'YES' : 'NO')?>
        </tr>
        <tr>
            <?= Chitch\tree('td', 'Session Extension', $sessionext= extension_loaded('session'),  $sessionext ? 'YES' : 'NO')?>
        </tr>
...
<?= batch(
    ...
    function ($suffices, $name) {
        return $suffices ? "<dt><a href='https://php.net/$name'>$name</a></dt><dd>installed YES</dd>" : "<dt><a href='https://php.net/$name'>$name</a></dt><dd>not installed ❌</dd>";
    }
)
?>
</dl>
...
if ($success) {
    ...
    chitchmail(
        $post['email'],
        'Chitch Install',
        'You have been added as the first user to Chitch. Your username is ' . $post['username'] . ". It is recommended to enable two-factor authentication for your email for more security.",
