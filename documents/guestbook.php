...
use function Chitch\{read, write, blockquote, wrap};
...
<label>Quote:
    <?php
    $quote = trim( substr( filter_input(INPUT_POST, 'quote', FILTER_SANITIZE_FULL_SPECIAL_CHARS) ?? '', 0, 500));
    ?>
...
<label>Alias:
    <?php
    $alias = trim( substr(filter_input(INPUT_POST, 'alias', FILTER_SANITIZE_FULL_SPECIAL_CHARS) ?? '', 0, 50));
    ?>
...
<p><ins>
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && !isset($error)) {
        if ($ishuman && $quote && $alias) {
            write('guestbook', $content = wrap('figure',
                blockquote($quote) . '- ' . wrap('figcaption', $alias)
                )
            );
