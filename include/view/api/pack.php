...
$zip = build('build/chitch.zip', fn($out, $in) => `(zip --recurse-paths --quiet "$out" $in)` ,'include');
$project = build('build/chitch.tar.zst' , fn($out, $in) => `tar -caf  $out $in`, 'include license.txt', 'bin');

# Block size of average file size
$image = build('build/chitch.sqsh', fn($out, $in) => `mksquashfs $in $out -comp zstd -b 256K`, 'include');
